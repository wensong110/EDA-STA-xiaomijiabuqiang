...
struct Advancer {//任务执行的策略
	...
	void dump(Subflow& subflow) {//忽略时钟偏斜，按end寄存器时钟来处理
		...
		if(clockPathRecord->pathRecord.begin()->index==temp.pathRecord.begin()->index) return;
		if(g->N->at(temp.pathRecord.begin()->index).IsCLK) return;
...
void Find(Graph& g, ST* x) {
	...
	task.wait_for_tasks();
	ClockTranceSet=new std::set<int>[g.N->size()+1];
	for(int i=1;i<g.N->size();i++){
...
void solve() {
	...
	Graph& g = *gg;
	string path = "./testcase_update20211029/testdata_8/";
	...
	cout<<"-------------------------\n";
	Find(g,(ST2*)0);
	cout<<"Setup Slack: "<<SetupSlack<<endl;
	cout<<"Hold Slack: "<<HoldSlack<<endl;
	cout<<"Combinal: " << CombinalSlack << endl;
	int pathcnt=0;
	for(const auto& x:SetupPath.container){
		cout<<"path:"<<++pathcnt<<endl;
		cout<<"\tdata arrival time:\n";
		int index;
		...
		for(const auto& y:x.pathRecord){
			...
			cout<<"\t"<<g.N->at(y.index).ID<<" "<<y.add<<" "<<y.time<<" "<<y.discribe<<endl;
		}
		cout<<"\tTC:\n";
		for(const auto& y:allClockPath[beginIndex].pathRecord){
			cout<<"\t"<<g.N->at(y.index).ID<<" "<<y.add<<" "<<y.time<<" "<<y.discribe<<endl;
		}
		cout<<"\tdata expected time:\n";
		cout<<"\t"<<g.N->at(index).ClockName<<" rise edge "<<g.N->at(index).clock<<endl;
		double clocknow=g.N->at(index).clock;
		for(const auto& y:x.clockRecord){
			cout<<"\t"<<g.N->at(y.index).ID<<" "<<y.add<<" "<<clocknow+y.add<<" "<<y.discribe<<endl;
			clocknow+=y.add;
		}
		cout<<"\t"<<g.N->at(index).ID<<" "<<"Tsu -1.0 "<<clocknow-1.0<<endl;
		cout<<"--------------------------------\n";
		cout<<"setup slack "<<x.time;
		cout<<endl;
		cout<<"============================================================================================\n";
	}
	for(const auto& x:HoldPath.container){
		...
		cout<<"\t"<<g.N->at(index).ID<<" "<<"Thd +1.0 "<<clocknow+1.0<<endl;
		cout<<"--------------------------------\n";
		cout<<"hold slack "<<x.time;
		cout<<endl;
		cout<<"============================================================================================\n";
	}
	for(const auto& x:CombinalPath.container){
		...
		bool first=1;
		for(const auto& y:x.pathRecord){
			cout<<"\t"<<g.N->at(y.index).ID<<" "<<(double)(first? 0.0:y.add )<<" "<<clocknow+y.add<<" "<<y.discribe<<endl;
			first=0;
			clocknow+=y.add;
		}
		cout<<"--------------------------------\n";
		cout<<"Combinal Port Delay: "<<clocknow;
		cout<<endl;
		cout<<"============================================================================================\n";
	}
	delete[] allClockPath;
